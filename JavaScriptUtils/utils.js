...
function deepClone(origin, target) {
	...
	for (var key in origin) {
		if (origin.hasOwnProperty(key)) {	// 排除原型上的属性
			if (typeof origin[key] === 'object' && origin[key] !== null) {	// 判断对象
				if (toStr.call(origin[key]) == '[object Array]') {	// 判断数组
					target[key] = [];
				} else {
					target[key] = {};
				}
...
Element.prototype.dragNclick = function (oMenu, elemClick) {
	...
	function dragnclick() {
		...
		addEvent(this, 'mousedown', function (event) {
			var e = event || window.event,
				btnCode = e.button,
				originOffset = {
...
// promisify封装（异步promise请求）：
function promisify(asyncFn) {
	return function (...args) {
		return new Promise(function (resolve, reject) {
			asyncFn(...args, (error, data) => {
